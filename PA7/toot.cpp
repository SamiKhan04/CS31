...
int City::nTootersAt(int r, int c) const
{
    ...
    for (int i = 0; i < m_nTooters; ++i) {
        const Tooter* toot = m_tooters[i];
        if (toot->row() == r && toot->col() == c) {
            nTooters++;
        }
    }
    return nTooters;
...
void City::preachToTootersAroundPlayer()
{
      ...
      for (int i = 0; i < m_nTooters;) {
        Tooter* toot = m_tooters[i];
        int rowd = toot->row() - m_player->row();
        int cold = toot->col() - m_player->col();
        if (rowd >= -1 && rowd <= 1 && cold >= -1 && cold <= 1 && (randInt(1,3) <= 2)) {
            delete m_tooters[i];
...
void City::moveTooters()
{
    for (int k = 0; k < m_nTooters; k++)
    {
        ...
        int rowd = toot->row() - m_player->row();
        int cold = toot->col() - m_player->col();
        if (rowd == 0 && (cold == 1 || cold == -1) || cold == 0 && (rowd == 1 || rowd == -1)) {
            m_player->getGassed();
        }
    }
}
